...
class Highstock(object):
    ...
    # class
    CHART_FILENAME = None
    template_environment = Environment(lstrip_blocks=True,
                                       trim_blocks=True,
                                       loader=pl)
    ...
    def __init__(self, **kwargs):
        ...
        ]  # DEM 2017/04/25: List of JSON data sources
        ...
        # Process kwargs
        allowed_kwargs = [
            "width", "height", "renderTo", "backgroundColor", "events",
    ...
    def add_data_set(self, data, series_type="line", name=None, **kwargs):
        ...
        series_data.__options__().update(
            SeriesOptions(series_type=series_type, **kwargs).__options__())
        self.data_temp.append(series_data)

    def add_data_from_jsonp(self,
                            data_src,
                            data_name='json_data',
                            series_type="line",
                            name=None,
        ...
        self.add_data_set(RawJavaScriptText(self.jsonp_data),
                          series_type,
                          name=name,
                          **kwargs)
    ...
    def add_navi_series_from_jsonp(self,
                                   data_src=None,
                                   data_name='json_data',
                                   series_type="line",
                                   **kwargs):
        ...
        self.add_navi_series(RawJavaScriptText(self.jsonp_data), series_type,
                             **kwargs)
    ...
    def set_options(self, option_type, option_dict, force_options=False):
        ...
        if force_options:
        ...
        elif (option_type == 'yAxis' or option_type == 'xAxis') and isinstance(
                option_dict, list):
            # For multi-Axis
            ...
            self.options["colors"].set_colors(
                option_dict)  # option_dict should be a list
    ...
    def buildcontent(self):
        ...
        self.data_list = [
            json.dumps(x, cls=HighchartsEncoder) for x in self.data_temp
        ]

        if self.navi_seri_flag:
            self.navi_seri = json.dumps(self.navi_seri_temp,
                                        cls=HighchartsEncoder)

        self._htmlcontent = self.template_content_highcharts.render(
            chart=self).encode('utf-8')
    ...
    def buildhtml(self):
        ...
        self.content = self._htmlcontent.decode(
            'utf-8')  # need to ensure unicode
    ...
    def iframe(self):
        ...
        width = int(self.options['chart'].__dict__['width']
                    ) if self.options['chart'].__dict__.get('width') else 960
        height = int(self.options['chart'].__dict__['height']
                     ) if self.options['chart'].__dict__.get('height') else 420
        ...
        if self.options['chart'].__dict__.get('options3d'):
            if len(htmlsrcdoc) < 99965000:
                ...
                return '<iframe style="border:0;outline:none;overflow:hidden" srcdoc="' + htmlsrcdoc + '" height=' + str(
                    height) + ' width=' + str(width) + '></iframe>'
        else:
            return '<iframe style="border:0;outline:none;overflow:hidden" srcdoc="' + htmlsrcdoc + '" height=' + str(
                height) + ' width=' + str(width) + '></iframe>'
...
class HighchartsEncoder(json.JSONEncoder):
    ...
    def default(self, obj):
        if isinstance(obj, RawJavaScriptText):
            ...
            obj = (
                u"Date.UTC({year},{month},{day},{hours},{minutes},{seconds},{millisec})"
                .format(year=utc[0],
                        month=utc[1] - 1,
                        day=utc[2],
                        hours=utc[3],
        ...
        elif isinstance(obj, CSSObject) or isinstance(
                obj, Formatter) or isinstance(obj, JSfunction):
            return obj.__jsonable__()
        elif isinstance(obj, SeriesOptions) or isinstance(obj, Series):
            return obj.__jsonable__()
        elif isinstance(obj, CommonObject) or isinstance(
                obj, ArrayObject) or isinstance(obj, ColorObject):
            return obj.__jsonable__()
        else:
